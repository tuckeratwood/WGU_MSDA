...
ggplot(churn, aes(Population, Churn)) + geom_jitter() + labs(title = "Jitterplot of Population vs. Churn")
ggplot(churn, aes(Children, Churn)) + geom_jitter()  + labs(title = "Jitterplot of Children vs. Churn") +
  scale_x_continuous(breaks = seq(0,10,1))
ggplot(churn, aes(Age, Churn)) + geom_jitter() + labs(title = "Jitterplot of Age vs. Churn")
ggplot(churn, aes(Income, Churn)) + geom_jitter() + labs(title = "Jitterplot of Income vs. Churn")
ggplot(churn, aes(Outage_sec_perweek, Churn)) + geom_jitter() + labs(title = "Jitterplot of Outage Seconds Per Week vs. Churn") +
  xlab("Outage Seconds Per Week")
ggplot(churn, aes(Email, Churn)) + geom_jitter() + labs(title = "Jitterplot of Email vs. Churn") + 
  scale_x_continuous(breaks = seq(0,24,2))
ggplot(churn, aes(Yearly_equip_failure, Churn)) + geom_jitter() + labs(title = "Jitterplot of Yearly Equipment Failure vs. Churn") + 
  scale_x_continuous(breaks = seq(0,6,1)) + xlab("Yearly Equipment Failures")
ggplot(churn, aes(MonthlyCharge, Churn)) + geom_jitter() + labs(title = "Jitterplot of Monthly Charge vs. Churn") +
  xlab("Monthly Charge")
ggplot(churn, aes(Bandwidth_GB_Year, Churn)) + geom_jitter() + labs(title = "Jitterplot of Bandwidth Gigabytes Per Year vs. Tenure") +
  xlab("Bandwidth Gigabytes Per Year")
ggplot(churn, aes(Tenure, Churn)) + geom_jitter() + labs(title = "Jitterplot of Tenure vs. Churn")
ggplot(churn, aes(x = Contract, fill = Churn)) + geom_bar(position = "dodge") +
  labs(title = "Grouped Bar Graph of Contract vs. Churn") + ylab("Number of Customers")
ggplot(churn, aes(x = DeviceProtection, fill = Churn)) + geom_bar(position = "dodge") +
  labs(title = "Grouped Bar Graph of Device Protection vs. Churn") + ylab("Number of Customers")
ggplot(churn, aes(x = TechSupport, fill = Churn)) + geom_bar(position = "dodge") +
  labs(title = "Grouped Bar Graph of Tech Support vs. Churn") + ylab("Number of Customers")
ggplot(churn, aes(x = InternetService, fill = Churn)) + geom_bar(position = "dodge") +
  labs(title = "Grouped Bar Graph of Internet Service vs. Churn") + ylab("Number of Customers")
...
write.csv(churn, "C:/Users/atwoo/OneDrive/Desktop/MSDADirectory/churn_new_task2.csv")
# saving prepared dataset as csv

# CREATING INITIAL MODEL:

in_model <- glm(Churn ~ Population + Children + Age + Income + Tenure + Outage_sec_perweek + Email + Yearly_equip_failure +
...
re_model <- glm(Churn ~ Population + Children + Age + Income + Tenure + Email + Yearly_equip_failure +
                  DeviceProtection + TechSupport + MonthlyCharge + Bandwidth_GB_Year + InternetService_Fiber_Optic +
                  InternetService_DSL + Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
...
re_model <- glm(Churn ~ Population + Children + Age + Income + Tenure + Email + DeviceProtection + TechSupport +
...
re_model <- glm(Churn ~ Population + Children + Age + Income + Tenure + DeviceProtection + TechSupport +
                  MonthlyCharge + Bandwidth_GB_Year + InternetService_Fiber_Optic + InternetService_DSL +
                  Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing Email
...
re_model <- glm(Churn ~ Population + Children + Age + Tenure + DeviceProtection + TechSupport +
                  MonthlyCharge + Bandwidth_GB_Year + InternetService_Fiber_Optic + InternetService_DSL +
                  Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing Income
...
re_model <- glm(Churn ~ Children + Age + Tenure + DeviceProtection + TechSupport + MonthlyCharge + Bandwidth_GB_Year +
                  InternetService_Fiber_Optic + InternetService_DSL + Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing Population

summary(re_model)

re_model <- glm(Churn ~ Children + Age + Tenure + DeviceProtection + TechSupport + MonthlyCharge + Bandwidth_GB_Year +
                  InternetService_Fiber_Optic + Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing InternetService_DSL

summary(re_model)

re_model <- glm(Churn ~ Age + Tenure + DeviceProtection + TechSupport + MonthlyCharge + Bandwidth_GB_Year +
                  InternetService_Fiber_Optic + Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing Children
...
re_model <- glm(Churn ~ Tenure + DeviceProtection + TechSupport + MonthlyCharge + InternetService_Fiber_Optic +
                  Contract_One_Year + Contract_Two_Year, churn, family = "binomial")
# removing Age
...
library(yardstick) # using yardstick package
actual_responses <- churn$Churn
predicted_responses_initial_model <- round(fitted(in_model))
in_outcomes <- table(predicted_responses_initial_model, actual_responses)
in_conf <- conf_mat(in_outcomes)
# creating confusion matrix for initial model

predicted_responses_reduced_model <- round(fitted(re_model))
re_outcomes <- table(predicted_responses_reduced_model, actual_responses)
re_conf <- conf_mat(re_outcomes)
# creating confusion matrix for reduced model

in_conf # initial confusion matrix
re_conf # reduced confusion matrix

summary(in_conf) # summary metrics for initial confusion matrix
summary(re_conf) # summary metrics for reduced confusion matrix

with(summary(in_model), 1 - deviance/null.deviance)
with(summary(re_model), 1 - deviance/null.deviance)
# calculating McFadden's R-squared for each model
# code from https://www.statology.org/glm-r-squared/
